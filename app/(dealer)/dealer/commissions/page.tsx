...
export default function DealerCommissionsPage() {
  ...
  useEffect(() => {
    const fetchCommissions = async () => {
      ...
      try {
        ...
        // Fetch commissions for this dealer
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'pending': return 'bg-amber text-amber-400 border-amber-500/20';
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-center min-h-[400px]">
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-zinc-400">Loading commissions...</p>
          </div>
        </div>
      </div>
    );
      ...
      <AnimatedContent>
        <div className="mb-8">
          ...
          <p className="text-zinc-400">
            Your commission ledger: track earnings and payout history
          </p>
        </div>
      </AnimatedContent>
                  ...
                  <table className="w-full">
                    <thead className="bg-zinc-800/30 border-b border-zinc-800">
                      <tr>
                        ...
                        <th className="text-left text-xs font-medium text-zinc-500 uppercase tracking-wider px-6 py-4 hidden sm:table-cell">Product</th>
                      ...
                      {filteredCommissions.map((commission) => (
                        <tr key={commission.id} className="hover:bg-zinc-800/20 transition-colors">
                          <td className="px-6 py-4">
                            <div className="text-sm font-medium text-zinc-100">{commission.date}</div>
                            {commission.paidDate && (
                              <div className="text-xs text-emerald-400">Paid: {commission.paidDate}</div>
                            )}
                          </td>
                          <td className="px-6 py-4">
                            <div className="text-sm font-medium text-zinc-300">{commission.id}</div>
                            <div className="text-xs text-zinc-500">{commission.orderId}</div>
                          </td>
                          <td className="px-6 py-4 hidden sm:table-cell">
                            <div className="text-sm text-zinc-300 max-w-xs truncate">{commission.product}</div>
                            <div className="text-xs text-zinc-500">{commission.customer}</div>
                          </td>
                          <td className="px-6 py-4">
