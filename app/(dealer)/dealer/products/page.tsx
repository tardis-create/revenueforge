    ...
    const fetchProducts = async () => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          const transformed: Product[] = apiProducts.map((p: ApiProduct) => ({
            id: p.id || p.sku || '',
            name: p.name || '',
            description: p.description || '',
            category: p.category || 'General',
            retailPrice: typeof p.price === 'number' ? p.price : parseFloat(String(p.price)) || 0,
            dealerPrice: typeof p.dealer_price === 'number' ? p.dealer_price : Math.round((typeof p.price === 'number' ? p.price : parseFloat(String(p.price)) || 0) * 0.75),
            commission: typeof p.commission === 'number' ? p.commission : Math.round((typeof p.price === 'number' ? p.price : parseFloat(String(p.price)) || 0) * 0.1),
            inStock: p.in_stock !== false && p.stock_quantity !== 0,
            image: p.image_url,
          }))
          setProducts(transformed.length > 0 ? transformed : getMockProducts())
        } else {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
        {filteredProducts.map((product, i) => (
          <AnimatedContent key={product.id} delay={0.05 * (i % 3)}>
            <SpotlightCard
              className={`bg-zinc-900/60 border border-zinc-800/50 rounded-xl overflow-hidden backdrop-blur-sm ${!product.inStock ? 'opacity-60' : ''}`}
      ...
      <AnimatedContent delay={0.3}>
        <div className="bg-gradient-to-br from-zinc-900 to-zinc-800 border border-zinc-800/50 rounded-xl p-6">
...
function getMockProducts(): Product[] {
  return [
    { id: 'PROD-001', name: 'Enterprise Analytics Suite', description: 'Comprehensive analytics platform for large-scale data processing.', category: 'Analytics', retailPrice: 5999, dealerPrice: 4500, commission: 450, inStock: true },
